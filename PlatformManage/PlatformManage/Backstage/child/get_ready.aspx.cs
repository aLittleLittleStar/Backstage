...
namespace PlatformManage.Backstage.child {
    public partial class get_ready : System.Web.UI.Page {
        ...
        private string[] other_cabinet_column = { "序号", "合同编号", "项目", "业主", "水槽、电器", "五金配件" };
        ...
        private void create_cmd(ref MySqlCmd.MySqlContext udata) {
            udata.context = "UPDATE ORDER_FORM SET `" + Database.convert_columns_name(this.waterchannel_wiring.ID)
                            + "`=\"" + this.waterchannel_wiring.Value
                            + "\", `" + Database.convert_columns_name(this.hardware_fitting.ID)
                            + "`=\"" + this.hardware_fitting.Value
