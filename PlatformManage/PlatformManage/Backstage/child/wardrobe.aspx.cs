...
namespace PlatformManage.Backstage.child {
    public partial class wardrobe : System.Web.UI.Page {
        ...
        private string[] other_cabinet_column = { "序号", "合同编号", "项目", "业主", "衣柜预定安装日期", "衣柜下单时间" };
        ...
        private void create_cmd(ref MySqlCmd.MySqlContext udata) {
            udata.context = "UPDATE ORDER_FORM SET `" + Database.convert_columns_name(this.clothes_board_reserve_time.ID)
                            + "`=\"" + this.clothes_board_reserve_time.Value
                            + "\", `" + Database.convert_columns_name(this.clothes_board_order_time.ID)
